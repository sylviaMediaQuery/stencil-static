...
const config = require('./config-aa114654.js');
const store = require('./store-56322166.js');
...
const NotificationMessage = class {
  ...
  render() {
    return index.h("div", { class: `notification ${this.type} ${this.visible ? 'show' : 'hide'}` }, this.message);
  }
};
...
const PayCargoCartComponent = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.cancel = index.createEvent(this, "paycargoCartClose", 6);
    this.onSuccess = index.createEvent(this, "paycargoPaymentSuccess", 6);
    this._defaultFields = {
      direction: 'Inbound',
    ...
    this.setApiHost = async (data) => {
      ...
      if (typeof data === 'string') {
        ...
        configObj.apiHost = (_a = config.config.apiHostsOptions[configObj === null || configObj === void 0 ? void 0 : configObj.env]) !== null && _a !== void 0 ? _a : config.config.apiHostsOptions['dev'];
        let key;
        for (key in configObj) {
          store.state[key] = configObj[key];
        }
      }
    };
    this.getFees = async () => {
      ...
      this.transactionFees = await getTransactionFees({
        ...
        total,
        transactionId,
        payerId: (store.state === null || store.state === void 0 ? void 0 : store.state.idToken) && store.state.idToken !== '' ? null : 1,
        paymentMethod: 'CREDITCARD',
        idToken: (store.state === null || store.state === void 0 ? void 0 : store.state.idToken) && store.state.idToken !== '' ? store.state.idToken : '',
        developerToken: store.state.developerToken,
        apiHost: store.state.apiHost,
        oneTimePayment: (store.state === null || store.state === void 0 ? void 0 : store.state.idToken) === '' ? true : false
      });
      console.log({ value: store.state.idToken, type: typeof store.state.idToken, compare: '' === store.state.idToken });
  ...
  developerTokenHandler(token) {
    if (!store.state.developerToken)
      store.state.developerToken = token;
    else if (store.state.developerToken !== token) {
      store.state.developerToken = token;
    }
  ...
  async dataDidChangeHandler(newValue) {
    ...
    if (vendorId) {
      const details = await getVendorsDetails({ apiHost: store.state.apiHost, vendorId: this._pcData.vendorId });
      if (details === null || details === void 0 ? void 0 : details.vendorId) {
        ...
        if (this.selectedRegion !== this.vendorDetails.region && this.vendorDetails.region) {
          ...
          this.bluesnapToken = await getBluesnapToken({ apiHost: store.state.apiHost, region: this.selectedRegion.toUpperCase() });
  ...
  componentWillLoad() {
    ...
    (async () => {
      ...
      try {
        ...
        const regionsResp = await getRegions({ apiHost: store.state.apiHost });
        this.regions = [...regionsResp];
        this.uiCulture = await getUiCultureByRegion({ region: this.selectedRegion, apiHost: store.state.apiHost });
        const bluesnapHost = (_a = config.config.bluesnapURL[store.state === null || store.state === void 0 ? void 0 : store.state.env]) !== null && _a !== void 0 ? _a : config.config.bluesnapURL['dev'];
        ...
        script.onload = async () => {
          ...
          //const now = moment()
          this.bluesnapToken = await getBluesnapToken({ apiHost: store.state.apiHost, region: region.toUpperCase() });
  ...
  render() {
    ...
    return (index.h("div", null, index.h("div", { id: "paycargo-checkout", class: this.visible ? 'wrapper show' : 'wrapper' }, index.h("div", { class: "modal" }, index.h("div", null, " ", this.showApprovedNotification && (index.h("notification-message", { message: "Transaction Approved!", type: "success" }))), index.h("nav", { class: "navbar-items" }, index.h("div", { class: "navbar-logo" }), index.h("div", { class: "close" }, index.h("button", { class: "close-button", onClick: () => this.close() }, index.h("img", { src: "https://d169y5idz4jz14.cloudfront.net/Assets/Icons/exit_icon_white.svg", alt: "exit" })))), index.h("div", { class: "transaction-container" }, this._pcData && (index.h("table", { class: "" }, index.h("thead", null, index.h("tr", null, index.h("th", { colSpan: 1, class: "border-bottom text-left", id: "payment-summary" }, "Payment Summary"), index.h("th", { colSpan: 2, class: "type-number border-bottom text-right blue-text" }, `${this._pcData.type.toUpperCase()}: #${this._pcData.number}`)), index.h("tr", { style: { height: '30px' } }, index.h("th", { colSpan: 1, class: "text-left", id: "vendor-line" }, "Vendor"), index.h("th", { colSpan: 2, class: "text-right blue-text" }, (_a = this.vendorDetails) === null || _a === void 0 ? void 0 : _a.name))), index.h("tbody", null, index.h("tr", null, index.h("td", { class: "border-bottom" }, " Item "), index.h("td", { class: "border-bottom text-right" }, " Rate "), index.h("td", { class: "border-bottom text-right" }, " Amount ")), index.h("tr", null, index.h("td", { class: "gray-text" }, " PayCargo Transaction "), index.h("td", { class: "text-right gray-text" }, ((_b = this.transactionFees) === null || _b === void 0 ? void 0 : _b.amount)
      ? currencyFormatter({ uiCulture: this.uiCulture, number: this.transactionFees.amount })
      : ''), index.h("td", { class: "text-right gray-text" }, ((_c = this.transactionFees) === null || _c === void 0 ? void 0 : _c.amount)
      ? currencyFormatter({ uiCulture: this.uiCulture, number: this.transactionFees.amount })
      : '')), index.h("tr", null, index.h("td", { class: "gray-text" }, " Shipper Fee "), index.h("td", { class: "text-right gray-text" }, ((_d = this.transactionFees) === null || _d === void 0 ? void 0 : _d.transactionFee)
      ? currencyFormatter({ uiCulture: this.uiCulture, number: this.transactionFees.transactionFee })
      : ''), index.h("td", { class: "text-right gray-text" }, ((_e = this.transactionFees) === null || _e === void 0 ? void 0 : _e.transactionFee)
      ? currencyFormatter({ uiCulture: this.uiCulture, number: this.transactionFees.transactionFee })
      : '')), index.h("tr", null, index.h("td", { class: "text-right", colSpan: 2 }, "Subtotal"), index.h("td", { class: "text-right" }, ((_f = this.transactionFees) === null || _f === void 0 ? void 0 : _f.transactionFee) && ((_g = this.transactionFees) === null || _g === void 0 ? void 0 : _g.amount)
      ? currencyFormatter({
        uiCulture: this.uiCulture,
        number: this.transactionFees.transactionFee + this.transactionFees.amount
      })
      : '')), index.h("tr", null, index.h("td", { class: "border-bottom text-left orange-text" }, " Credit Card Processing Fee "), index.h("td", { class: "border-bottom text-right orange-text" }, ' ', `3.5% x ${((_h = this.transactionFees) === null || _h === void 0 ? void 0 : _h.transactionFee) && ((_j = this.transactionFees) === null || _j === void 0 ? void 0 : _j.amount)
      ? currencyFormatter({
        uiCulture: this.uiCulture,
        number: this.transactionFees.transactionFee + this.transactionFees.amount
      })
      : ''}`, ' '), index.h("td", { class: "border-bottom text-right orange-text" }, ((_k = this.transactionFees) === null || _k === void 0 ? void 0 : _k.processingFee)
      ? currencyFormatter({ uiCulture: this.uiCulture, number: this.transactionFees.processingFee })
      : '')), index.h("tr", null, index.h("td", { colSpan: 2, class: "text-right blue-text" }, ' ', "Total:", ' '), index.h("td", { class: "text-right blue-text" }, ((_l = this.transactionFees) === null || _l === void 0 ? void 0 : _l.totalWithFees)
      ? currencyFormatter({ uiCulture: this.uiCulture, number: this.transactionFees.totalWithFees })
      : '')))))), index.h("div", { class: "bluesnap-container" }, index.h("div", { class: "card" }, index.h("form", { onSubmit: event => this.checkout(event),
      // onChange={() => this.validateForm()}
      id: "ccForm" }, index.h("div", { class: "row" }, index.h("div", { class: "form-group" }, index.h("div", null, index.h("div", { class: "label gray-text" }, index.h("label", { class: "required", htmlFor: "email" }, "Email")), index.h("input", { type: "email", id: "email", name: "email", class: "input", value: this.userData.email, onInput: event => this.handleChange(event), placeholder: "janedoe@email.com", required: true })))), index.h("div", { class: "row" }, index.h("div", { class: "name-container" }, index.h("div", { class: "form-group" }, index.h("div", { class: "label gray-text" }, index.h("label", { class: "required", htmlFor: "first-name" }, "First Name")), index.h("input", { required: true, class: "input", id: "first-name", name: "firstName", type: "text", value: this.userData.firstName, onInput: event => this.handleChange(event), placeholder: "Jane" })), index.h("div", { class: "form-group" }, index.h("div", { class: "label gray-text" }, index.h("label", { class: "required", htmlFor: "last-name" }, "Last Name")), index.h("input", { required: true, id: "last-name", class: "input", name: "lastName", type: "text", value: this.userData.lastName, onInput: event => this.handleChange(event), placeholder: "Doe" })))), index.h("div", { class: "row" }, index.h("div", { class: "name-container" }, index.h("div", { class: "form-group" }, index.h("div", { class: "label" }, index.h("label", { htmlFor: "phone", class: "required" }, "Phone")), index.h("input", { required: true, class: "input", id: "phone", name: "phone", type: "tel", value: this.userData.phone, onInput: event => this.handleChange(event), placeholder: "(123) 456-7890", minlength: "10", maxlength: "10" })), index.h("div", { class: "form-group" }, index.h("div", { class: "label" }, index.h("label", { htmlFor: "zip", class: "required" }, "Region")), index.h("div", { class: "" }, index.h("select", { class: "input-field", id: "regions", required: true, onChange: event => this.setRegion(event), disabled: true }, this.regions.map(({ regionId, regionName, defaultCurrencyId }) => {
      return (index.h("option", { selected: regionId === this.selectedRegion, value: regionId, key: regionId }, `${regionName} - ${defaultCurrencyId}`));
    }))))), index.h("div", { class: "row" }, index.h("div", { class: "form-group" }, index.h("div", { class: "label" }, index.h("label", { htmlFor: "company-name" }, " Company name ")), index.h("input", { class: "input", type: "text", id: "company-name", name: "companyName", value: this.userData.companyName, onInput: event => this.handleChange(event), placeholder: "Jane Doe Company" }))), index.h("div", { class: "form-group", id: "card-info-container" }, index.h("label", { id: "card-information", class: "required" }, "Credit Card Information"), index.h("div", { id: "pc-ccn", "data-bluesnap": "ccn", class: "input-field" }), index.h("div", { class: "cc-info-container" }, index.h("div", { id: "pc-exp", "data-bluesnap": "exp", class: "input-field" }), index.h("div", { id: "pc-cvv", "data-bluesnap": "cvv", class: "input-field" }), index.h("div", null, index.h("input", { id: "zip", name: "zip", required: true, type: "text", placeholder: "ZIP", value: this.userData.zip, onInput: event => this.handleChange(event), class: "input-field", minlength: "5", maxlength: "5" }))))), index.h("div", { class: "row", id: "errorMsg-and-button" }, index.h("div", { class: "text-left m-t-10" }, this.errorMsg && index.h("div", { class: "error" }, this.errorMsg)), index.h("div", { class: "text-right m-t-10" }, index.h("button", { class: "pc-payment-button", type: "submit", disabled: !this.validCcn || !this.validExp || !this.validCvv || !this.validTransaction }, this.isLoading ? index.h("loading-spinner", null) : 'Complete Payment'))))))))));
  }
...
exports.notification_message = NotificationMessage;
exports.paycargo_cart = PayCargoCartComponent;
